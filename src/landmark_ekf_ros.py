...
def cb_array(data):
    ...
    for i in range(n):
        ...
        if j>0:
            ...
            dis=dis+EKF_localization.get_dis(new_Z[0,0]*0.001,new_Z[1,0]*(-0.001),TREE_DATA[int(j-1)][0],TREE_DATA[int(j-1)][1])
            n_in=n_in+1
    ...
    if n_in==0:
        ...
        mean_error=dis/n_in
    m=Float64MultiArray(data=[dis,mean_error,n,n_in])
